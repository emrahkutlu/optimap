...
function getRailCoords(){
	...
	getGeojsonSubway(latLeftTopPlus,lonLeftTopPlus,latRightBtmPlus,lonRightBtmPlus);
}
...
/*			        if (typeof feature.properties.relations[i] !== "undefined"){
			        ...
			        //console.log(feature.properties.relations[1].reltags.ref)
			    console.log(feature)
			        //if (i){
/*			        if (k){
...
/*	console.log("lefttop.lat: " + lefttop.lat);
	...
	}if ((parseFloat(newLefttop.lat) >= parseFloat(latLeftTopPlus) || parseFloat(newLefttop.lng) <= parseFloat(lonLeftTopPlus) || parseFloat(newRightbottom.lat) <= parseFloat(latRightBtmPlus) || parseFloat(newRightbottom.lng) >= parseFloat(lonRightBtmPlus)) && map.getZoom() > 10){
		console.log(subwayLines)
		if (map.getZoom() > 10 && !map.hasLayer()){
			//getRailCoords();
			console.log("getRailCoords");
	}}
...
map.on('zoomend', function(){	//Wenn Zoom < x dann schalte Indoor-Anzeige aus und aktiviere wieder POI-Anzeige
	...
	if (map.getZoom() > 14 && !(map.hasLayer(stationPois)) && !(map.hasLayer(indoorLayer))){
		...
		map.removeLayer(stationPois);
		//stationPois.clearLayers();
	}if (map.getZoom() <= 10 && map.hasLayer(subwayLines)){

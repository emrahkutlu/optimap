...
// Aussehen der GeoJson Daten definieren
	...
	// Poppup mit Inhalt definieren
		...
		//console.log(feature.properties.tags.name);
			//Hilfsvariablen
			var test = feature.properties.tags
			if (test == "payment:credit_cards"){
				console.log(test)
			}
			//Merkmalsvariablen
			...
			var creditCards = feature.properties.tags["payment:credit_cards"]; // Zahlkarteninfo ist payment: !!!
...
/*				console.log("header: " + header);
				...
				var popupContentAmenity = '<caption><h5>' + 'Geldautomat' + '</h5></caption>';
				if (opening){
					opening =  '<tr><td><i>Offen: &nbsp;</i></td><td>' + opening + '</td></tr>';
				...
				var popupContentAmenity = '<caption><h5>' + 'Fahrscheinautomat' + '</h5></caption>';
				if (phone){
					phone = phone;
					popupContentAmenity += '<tr><td><i>Telefon:&nbsp;</i></td><td>' + phone + '</td></tr>';
				}
				...
				if (creditCards){
					popupContentAmenity += '<tr><td><i>Kreditkarte: &nbsp;</i></td><td>' + creditCards + '</td></tr>';
				}
				popupContentAmenity =  '<table>' + popupContentAmenity + '</table>';
				layer.bindPopup(popupContentAmenity);
			} // Ende Fahrscheinautomat
			else if (feature.properties.tags.vending === 'parcel'){	//Falls 'amenity-Tag' vorhanden
				var popupContentAmenity = '<caption><h5>' + 'Post & Paket' + '</h5></caption>';
				if (opening){
					opening =  '<tr><td><i>Offen: &nbsp;</i></td><td>' + opening + '</td></tr>';
					popupContentAmenity += opening;
				}
				...
				if (creditCards){
					popupContentAmenity += '<tr><td><i>Betreiber: &nbsp;</i></td><td>' + creditCards + '</td></tr>';
				}
				popupContentAmenity =  '<table>' + popupContentAmenity + '</table>';
				layer.bindPopup(popupContentAmenity);
			} // Ende Paketautomaten
			else if (feature.properties.tags.description){	//Falls 'description-Tag' vorhanden
				var popupContentDesc = '<caption><h5>' + description + '</h5></caption>';
				if (opening){
					opening =  '<tr><td><i>Offen: &nbsp;</i></td><td>' + opening + '</td></tr>';
...
//	Daten mit JQuery abgreifen und in geojson transformieren, stylen
	$.get("http://overpass-api.de/api/interpreter?data=" + query, function(data){
		...
		highWays = new L.Indoor(geoJSON, {
			...
			style: function(feature) {
				if (feature.properties.tags.highway === 'footway'){
					return {color: '#e4f3e4', weight: 3, opacity: 0.8};
				}
				if (feature.properties.tags.highway === 'steps'){
					return {color: '#e4f3e4', weight: 2, opacity: 0.8, dashArray: 3.3};
				}
			},
	//PointtoLayer macht das keine dominanten Marker für Punktobjekte ausgegeben werden!
			pointToLayer: function (feature, latlng){
...
//## Beginn UNTERE Highways-Ebene
	...
	highWaysD = new L.Indoor(geoJSONc, {
		...
		style: function(feature) {
			if (feature.properties.tags.highway === 'footway'){
				return {color: '#7fc084', weight: 3, opacity: 0.5};
			}
			if (feature.properties.tags.highway === 'steps'){
				return {color: '#7fc084', weight: 2, opacity: 0.5, dashArray: 3.3};
			}
		},
//PointtoLayer macht das keine dominanten Marker für Punktobjekte ausgegeben werden!
		pointToLayer: function (feature, latlng){

...
function getGeojsonLine(leftlat,leftlng,rightlat,rightlng){
    var 	query = '<union><query type="way"><has-kv k="railway" modv="" v="subway"/><bbox-query s="'+rightlat+'" n="'+leftlat+'" w="'+leftlng+'" e="'+rightlng+'"/></query><recurse type="down"/></union><print/><union><query type="relation"><has-kv k="railway" modv="" v="subway"/><bbox-query s="'+rightlat+'" n="'+leftlat+'" w="'+leftlng+'" e="'+rightlng+'"/></query><recurse type="down"/></union><print/>';
//	6.) Daten mit JQuery abgreifen und in geojson transformieren, stylen
	$.get("http://overpass-api.de/api/interpreter?data=" + query, function(data) {
            ...
            transportLines.addTo(map);
       		transportLines.on('click', onClick);
	});
	...
	function onClick(selPoi){
		...
		map.removeLayer(transportLines);	//StationPOIs daten löschen
		transportLines.clearLayers();
	}
}

...
$("#locate-btn").click(function() {
	...
	.bindPopup("ungefÃ¤hrer Radius: " + radius + " m").openPopup();
...
*	1. Variablem definieren fÃ¼r Link(Webadresse), Link(Kartendienst), Quellenangabe und fÃ¼r den Aufruf der Karte
...
//	3.)	<!-- Hier werden LAyer definiert- die in der Layer Ã¼bersicht erschienen ->
	var 	baseMaps = {
			"OSM Mapnik": osmMap,
			"OSM Grayscale": osmMapBW,
			"Mapbox streets": mapboxMap
			};
/*// HighWays Gruppe fÃ¼r korrestpondierende Ebenen
...
// Variable fÃ¼r den Status der Rolltreppen
var escState
...
//(GetGeoJsonData) Globale Variable fÃ¼r Level Angabe ...
var relLevel 	//aus Relationen
var tagsLevel	//aus Tags

// LayerGroup als Container fÃ¼r die folgenden GeoJSON Layer
	var 	jsonLayers = L.layerGroup();
	jsonLayers.addTo(map);
//	<!-- LayerschaltflÃ¤chen fÃ¼r Kartendienste -->
...
/*
* ##### POIs #####
* Daten Ã¼ber Overpass API abfragen und in GEOJSON Daten transformieren
*	4. Koordinaten werden abgegriffen fÃ¼r die Overpass API Abfrage
...
function getCoords(){
	...
	// Addiere/Subtrahiere Werte, damit nicht immer Daten  fÃ¼r GeoJSONPoi nachgeladen werden bei Kartenverschieben --> vergrÃ¶ÃŸerte BBOX
...
/*	console.log("lefttop.lat: " + lefttop.lat);
	...
	//if (map.getZoom() <= 16) {	// Abfrage prÃ¼ft ob rausgezoomt wurde, dann sollen POIs WIEDER angezeigt werden.
...
jsonLayers.addTo(map);
/*  #######################  Anfang U-Bahn  ######################  */
// Koordinatenabfrage seperat fÃ¼r Funktion getGeoJsonSubway
...
/*			        if (k){
				...
				//for(var i in relComp){
			        ...
			        }if (name.match('70')){ //Ab hier DÃ¼sseldorf
		...
		//der Karte hinzufÃ¼gen
		$("#loading").hide();
		//Direkt der Karte hinzufÃ¼gen:
  		//subwayLines.addTo(map);
  		//Indirekt (Ã¼ber Container):
...
/*  #######################  Anfang S-Bahn  ######################  */
// Koordinatenabfrage seperat fÃ¼r Funktion getGeoJsonSRail
...
/*  #######################  Anfang Tram  ######################  */
// Koordinatenabfrage seperat fÃ¼r Funktion getGeoJsonTram
...
/*  #######################  Anfang Bus  ######################  */
// Koordinatenabfrage seperat fÃ¼r Funktion getGeojsonBus
...
/*  #######################  Ende getGeoJsonSRail  ######################  */
//	5.)	<!-- Hier kommt die Abfrage fÃ¼r die Overpass-api in eine Variable--> // OFFEN: Anfrage verÃ¤ndern das Daten nicht bei jeder Bewegung abgefragt werden sondern fÃ¼r einen grÃ¶ÃŸeren Bereich!
				...
				// Poppup mit Inhalt definieren
					onEachFeature: function(feature, layer){
						...
						layer.on("mouseout", function (e){		// Poppup geschlossen wenn Mouseout
							layer.closePopup();		// Poppup schlieÃŸen
	          			});
					}
        		});
			// Marker der Karte hinzufÃ¼gen
            stationPois.addTo(map);
       		stationPois.on('click', onClick);
	});

	// Bei Klick zum POI zoomen und zentrieren, satioinPoi Daten lÃ¶schen
	function onClick(selPoi){
		map.setView(selPoi.latlng, 18);
		// Koordinaten fÃ¼r aktuellen Kartenausschnitt beziehen
		...
		map.removeLayer(stationPois);	//StationPOIs daten lÃ¶schen
...
* Daten Ã¼ber Overpass API abfragen und in GEOJSON Daten transformieren -> INdoor Daten
*/
...
/*			{
		...
		},	//PointtoLayer macht das keine dominanten Marker fÃ¼r Punktobjekte ausgegeben werden!
		pointToLayer: function (feature, latlng){
	...
	// Poppup mit Inhalt definieren
		...
		//console.log(feature.properties.tags.name);
			...
			var amenity = feature.properties.tags.amenity; // FÃ¼r Automaten
			// Zahlkarteninfo :
			...
			// ### ZahlmÃ¶glichkeiten Abfrage
				if (fee){
					paymModality += '<tr><td class="cellStyle"><i>GebÃ¼hrenpflichtig: &nbsp;</i></td><td>' + fee + '</td></tr>';
				}
				if (creditCards){
					paymModality += '<tr><td class="cellStyle"><i>Kreditkarte: &nbsp;</i></td><td>' + creditCards + '</td></tr>';
				}
				if (coins){
					paymModality += '<tr><td class="cellStyle"><i>MÃ¼nzen: &nbsp;</i></td><td>' + coins + '</td></tr>';
				}
				if (notes){
...
/*				console.log("header: " + header);
				...
				for (var i = 0; i < escArray.length; i++) {
					...
					if (escArray[i].contains(description)) {
						//console.log(i)  
...
/*			layer.on("mouseover", function (e){		// Poppup wird geladen wenn Mouseover
        ...
        //Ausstattung
	        ...
	        if (feature.properties.tags.conveying === 'forward'){
	        	rolltreppenSchleife(description);
    		}
    		...
    		if (feature.properties.tags.conveying === 'reversible'){
				...
				for (var i = 0; i < escArray.length; i++) {
					//IF-Abfrage um Array-Value zu bestimmen
					if (escArray[i].contains(feature.properties.tags.description)) {
						console.log(description)
						splitArrayString(escArray[i], ': ');
						escState2 = arrayStrings[2].slice(0, 10);
						console.log(escState2)
				    	if (escState2 !== 'in Betrieb') {
	...
	//stationPois.clearLayers();
// Indoor Layer auf null setzen und IndoorLayer zur Karte hinzufÃ¼gen
    indoorLayer.setLevel("0");
    indoorLayer.addTo(map);
    getGeojsonHighways(latLeftTopID, lonLeftTopID, latRightBtmID, lonRightBtmID);
    $("#loading").hide();
//LevelControl anlegen und mit Level info aus IndoorLayer fÃ¼llen
    levelControl = new L.Control.Level({
        level: "0",
        levels: indoorLayer.getLevels()
    });
// Level-SchaltflÃ¤chen mit Indoor-Layer verknÃ¼pfen
...
//wird bei Kartenbewegung aufgerufen, Check ob auÃŸerhalb von BBOX, wenn ja dann neue Daten laden (getCoords)
map.on('moveend', function(){
/*	console.log("lefttop.lat: " + lefttop.lat);
	console.log("lefttop.lng:  " + lefttop.lng);
	console.log("rightbottom.lat: " + rightbottom.lat);
	...
	// Wenn auÃŸerhalb des Koordinatenbereiches und Zoom > 14 dann getCoords
	if ((parseFloat(newLefttop.lat) >= parseFloat(latLeftTopPlus) || parseFloat(newLefttop.lng) <= parseFloat(lonLeftTopPlus) || parseFloat(newRightbottom.lat) <= parseFloat(latRightBtmPlus) || parseFloat(newRightbottom.lng) >= parseFloat(lonRightBtmPlus)) && map.getZoom() > 14){
		map.removeLayer(stationPois);
		stationPois.clearLayers();
		getCoords();
	...
	// Wenn Zoom <= 14 und stationpois vorhanden dann lÃ¶sche stationpois
	};
...
//wird bei Zoomende aufgerufen, testet ob MaÃŸstab zu klein, dann remove INDOOR
map.on('zoomend', function(){	//Wenn Zoom < x dann schalte Indoor-Anzeige aus und aktiviere wieder POI-Anzeige
	if (map.getZoom() <= 14 && map.hasLayer(stationPois)){
		...
		//stationPois.clearLayers();
	}	// Wenn Zoom > 14 und noch keine stationpois vorhanden, fÃ¼hre getCoords aus, -->> Instabil?
...
//	<!-- Geocoder SchaltflÃ¤che: FÃ¼r Suchen-Funktion. MÃ¶glichkeit in Klammer: collapsed: false, position: 'bottomright',text: 'Find!', ->
...
// Wird unter Style aufgerufen von Rolltreppen und Aufzug
function rolltreppenSchleife(descriptState){
	// Schleife um Rolltreppen-Array zu durchlaufen
	for (var i = 0; i < escArray.length; i++) {
		//IF-Abfrage um Array-Value zu bestimmen
		if (escArray[i].contains(descriptState)) {
			//console.log(description)
			splitArrayString(escArray[i], ': ');
			escState = arrayStrings[2].slice(0, 10);
			console.log(escState)
...
/*function escStateStyle(argument){
	if (argument != "in Betrieb") {
		return {'auÃŸer Betrieb'};
	} 
	if (argument === 'in Betrieb') {
    	return {'in Betrieb'};
	}

...
//	6.) Daten mit JQuery abgreifen und in geojson transformieren, stylen
	...
	function onClick(selPoi){
		...
		osmMap.setOpacity(0.3);
		mapboxMap.setOpacity(0.3);
	}
}
...
// Aussehen der GeoJson Daten definieren
        ...
        //Nicht ausgeben:
        	if (feature.properties.tags.waterway){
        		return {fillColor: 'none', color: 'none', weight: 1, fillOpacity: 0.9};
	        }
	    //...
        //Gebäude
        	if (feature.properties.tags.building === 'train_station'){
	            return {fillColor: '#d4d4d4', color: '#333333', weight: 1, fillOpacity: 0.9};
	        }
	        ...
	        if (typeof feature.properties.relations[0] !== 'undefined'){
	        	...
	        	if (feature.properties.tags.amenity === 'townhall'){
	            	return {fillColor: '#e9b8ff', color: '333333', weight: 1, fillOpacity: 0.5};
	        	}
	        	if (feature.properties.tags.building === 'roof'){
	            	return {fillColor: '#d4d4d4', color: '333333', weight: 1, fillOpacity: 0.5};
	        	}
	        }
	        if (feature.properties.tags.landuse === 'railway'){
	            return {fillColor: '#d4d4d4', color: 'none', weight: 1, fillOpacity: 0.5};
	        }
	        if (feature.properties.tags.amenity === 'townhall'){
	            return {fillColor: '#e9b8ff', color: '333333', weight: 1, fillOpacity: 0.5};
	        }//DAch
        ...
        //Ausstattung
        	...
        	if (feature.properties.tags.information === 'office'){
	            return {fillColor: '#ffffb5', color: '#FFFFFF', weight: 1, fillOpacity: 0.9};
            }
	        ...
	        if (feature.properties.tags.tunnel === 'yes'){
	            return {fillColor: '#f0f8ff', color: '#FFFFFF', weight: 2, fillOpacity: 0.6};
	        }	//Sicherheitsdienst
...
/*function actLevel(){
	console.log(levelControl.getLevel());
}*/
...
function getGeojsonHighways(leftlat,leftlng,rightlat,rightlng){
    var 	query = '<union><query type="node"><has-kv k="highway" modv="" v="footway"/><bbox-query s="'+rightlat+'" n="'+leftlat+'" w="'+leftlng+'" e="'+rightlng+'"/></query><query type="way"><has-kv k="highway" modv="" v="footway"/><bbox-query s="'+rightlat+'" n="'+leftlat+'" w="'+leftlng+'" e="'+rightlng+'"/></query><query type="relation"><has-kv k="highway" modv="" v="footway"/><bbox-query s="'+rightlat+'" n="'+leftlat+'" w="'+leftlng+'" e="'+rightlng+'"/></query></union><union><item/><recurse type="down"/><recurse type="way-relation"/><recurse type="node-relation"/></union><print/>';
//	Daten mit JQuery abgreifen und in geojson transformieren, stylen
	$.get("http://overpass-api.de/api/interpreter?data=" + query, function(data){
		...
		highWays = new L.Indoor(geoJSON, {
			...
			style: function(feature){
	        	if (feature.properties.tags.highway === 'footway'){
	        		return {fillColor: '#333333', color: '#ff8400', weight: 1, fillOpacity: 0.5};
		        }
		    },
	//PointtoLayer macht das keine dominanten Marker für Punktobjekte ausgegeben werden!
			pointToLayer: function (feature, latlng){
		...
		//der Karte hinzufügen
		//actLevel()
        //highWays.addTo(corLevels); // Fußwege nächstes Level
		levelControl.addEventListener("levelchange", actLevel);
			highWaysIni = $.extend({}, highWays);
			highWaysIni.setLevel("-1");
			highWaysIni.addTo(map);
			highWaysFollowing = $.extend({}, highWays);
			highWaysPreceding = $.extend({}, highWays);
		function actLevel(){
			//map.removeLayer(highWaysPreceding);
			//console.log(levelControl.getLevel());
			followingLevel = levelControl.getLevel() + 1;
			precedingLevel = levelControl.getLevel() - 1;
			//console.log(followingLevel);
			//console.log(precedingLevel);
			highWaysFollowing.setLevel(followingLevel);
			highWaysPreceding.setLevel(precedingLevel);
			//console.log(highWaysPreceding.getLevel());
			//corLevels.addTo(map);
			highWaysPreceding.addTo(map); // Fußwege vorangegangenes Level
			highWaysFollowing.addTo(map);
		}
		//levelControl.addEventListener("levelchange", highWaysFollowing.setLevel, highWaysFollowing);
	});
};

...
//	1.)
	var
//OSM
			osmLink = '<a href = "http://openstreetmap.org">OpenStreetMap</a>',		//<!--Link FÃœR Quellenangabe-->
			osmUrl = 'http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',			//<!--Kartenlink FÃœR Kartendienst-->
...
//OSM Grayscale
			//osmGrUrl = 'http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
			//osmMapGr = L.tileLayer.grayscale(osmBWUrl, {attribution: osmAttrib, maxZoom: 20, maxNativeZoom:18}),			
//Mapbox
			mapboxLink = '<a href = "http://mapbox.com">Mapbox</a>',
			mapboxUrl = 'https://a.tiles.mapbox.com/v3/rotkelch.ib6amnh3/{z}/{x}/{y}.png',
			mapboxAttrib = 'Map data &copy; <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery Â© <a href="http://mapbox.com">Mapbox</a>',
...
//	3.)	<!-- Hier werden LAyer definiert- die in der Layer übersicht erschienen ->
	var 	baseMaps = {
			"OSM Mapnik": osmMap,
			"OSM Mapnik BW": osmMapBW,
			//"OSM Grayscale": osmMapGr,
...
// Variable für den Status der escalatorData
var escState
	...
	$.get("http://overpass-api.de/api/interpreter?data=" + query, function(data){
		...
		var geoJson = osmtogeojson(data);
			...
			// Poppup mit Inhalt definieren
		        ...
		        //U-Bahn-Linien
		        // Hier muss eine Funktion hin die alle vorkommenden Namen aufgreift und Farben zuweist, damit diese Darstellung Ãœberall klappt
...
//	6.) Daten mit JQuery abgreifen und in geojson transformieren, stylen
	...
	function onClick(selPoi){
		escalatorData(selPoi.layer.feature.properties.tags.name);
		map.setView(selPoi.latlng, 19);
		...
		osmMapBW.setOpacity(0.2);
 		...
 		// Beziehe Rolltreppen Informationen
 		//console.log(selPoi.layer.feature.properties.tags.name)
	}
}
...
/*    if(feat.properties.tags.description === 'steps'){
    ...
    //console.log(lalo)
    //console.log(feat)
    if(feat.properties.tags.highway === 'elevator' && feat.properties.tags.building === 'yes'){
    	return getEscalatorStatus(feat, lalo)
    }if(feat.properties.tags.highway === 'lift'){
		return L.marker(lalo, {icon: wheelchair})
/*    }if(feat.properties.tags.building === 'yes' && feat.properties.tags.highway === 'elevator'){
    	return L.marker(lalo, {icon: aufzugAnimation})*/
/*    }if(feat.properties.tags.railway === 'subway_entrance'){
    	return L.marker(lalo, {icon: entrance})*/
...
//console.log(leftlat,leftlng,rightlat,rightlng);
	...
	console.log(geoJSON);
	// ### Filter der Aufzugsflächen für Mittelpunktberechnung der Aufzugsflächen	
	var a = geoJSON;
	//console.log(a.features)
	a.features.forEach(function(entry) {
	//console.log(entry);
	filterElev = a.features.filter(function (el) {
  		return (el.geometry.type === "Polygon" && el.properties.tags.highway === "elevator");
	});
		//console.log(filterElev)
	});
	elevatorTest(filterElev);
	//console.log(filterElev)
	//console.log(a.features)

	// Hier sollte für Aufzugsflächen der Mittelpunkt berechnet werden und als Punkt zurückgeworfen werden
function elevatorTest(feature){
	//console.log(feature)
	for (i = 0; i < feature.length; i++) {
	//console.log(feature[i])

		var polygon = feature[i].geometry.coordinates[0];
		var getPolygonCenter = function (poly) {
		    return poly.reduce(function (x,y) {
		        return [x[0] + y[0]/poly.length, x[1] + y[1]/poly.length]}, [0,0]) 
		}

		var polygonCenter = getPolygonCenter(polygon);
		var polyCenterLon = polygonCenter[0].toFixed(7)
		var polyCenterLat = polygonCenter[1].toFixed(7)
		var polyCenter 	  = [parseFloat(polyCenterLon), parseFloat(polyCenterLat)]
		//console.log(polyCenter)
		var polygonCenterLat = polygonCenter[0].toFixed(7)
		var polygonCenterLng = polygonCenter[1].toFixed(7)
		//console.log(polygonCenterLng)
		var polygonCenterLatLng = L.latLng(polygonCenterLng, polygonCenterLat)
		//console.log(polygonCenterLatLng)
		//$.extend(feature.geometry.coordinates[0], polygonCenterLatLng)
		delete feature[i].geometry.coordinates[0]
		feature[i].geometry.coordinates = polyCenter;
		delete feature[i].geometry.type
		feature[i].geometry.type = "Point"
		delete feature[i].type
		feature[i].type = "Feature"
		delete feature[i].properties.type
		feature[i].properties.type = "node"
		//console.log(feature)

		//elevatorMarker = L.marker(polygonCenter, {icon: aufzugAnimation}); 
		//return feature;
		//return selectIndoorIcon(feature, polygonCenterLatLng)
		//return L.marker(polygonCenterLatLng, {icon: aufzugAnimation});
		//console.log(feature[i])
		//return feature[i];
...
//console.log(feature)
	//console.log(geoJSON.features[4].geometry.type === "Polygon")
...
/*			{
        ...
        //Ausstattung
	        ...
	        if (feature.properties.tags.conveying === 'forward'){
	        	return getEscalatorStatus(feature);
    		}
            ...
            if (feature.properties.tags.highway === 'elevator' && feature.properties.tags.building === 'yes' ){
	        	return {fillColor: 'none', color: 'none', weight: 0, fillOpacity: 0};
	        	//return elevatorTest(feature);
	        	//return elevatorMarker;
	        	
	        	//return null //L.marker(feature.geometry.coordinates[0].getCenter());
	            //return getEscalatorStatus(feature);
        	}
	       if (feature.properties.tags.highway === 'elevator'){
	        	return {fillColor: 'none', color: 'none', weight: 0, fillOpacity: 0};
	        	//elevatorTest(feature);
	        	//return null //L.marker(feature.geometry.coordinates[0].getCenter());
	            //return getEscalatorStatus(feature);
        	}
		...
		//console.log(feature.properties.tags.name);
			//Hilfsvariablen
			var tagsAll = feature.properties.tags; 
			var paymModality = ''
			// ...
			//Merkmalsvariablen
...
/*				console.log("header: " + header);
				...
				// Anfang esclator description
			else if (description && (feature.properties.tags.conveying || feature.properties.tags.highway === 'elevator')){	//Rolltreppen
				escPopup(feature, layer);
				...
				//console.log("testo: " + testo + testo.length)
...
//	Daten mit JQuery abgreifen und in geojson transformieren, stylen
	...
	//PointtoLayer macht das keine dominanten Marker für Punktobjekte ausgegeben werden!
	    	...
	    	onEachFeature: function(feature, layer){
	    		if (feature.properties.tags.description && (feature.properties.tags.conveying || feature.properties.tags.elevator)){
	    		escPopup(feature, layer);
				layer.on('mouseover', function (e) {
				            this.openPopup();
				        });
...
// Wird unter Style aufgerufen von Rolltreppe und Aufzug, prüft auf ob in Betrieb oder nicht (mvg)
function getEscalatorStatus(feature, lalo){
	// Schleife um Rolltreppen-Array zu durchlaufen
	for (var i = 0; i < escArray.length; i++) {
		//IF-Abfrage um Array-Value zu bestimmen
		//console.log(escArray[3]);
		if (escArray[i].indexOf(feature.properties.tags.description)> -1) {
			//console.log(description)
			splitArrayString(escArray[i], ': ');
			escState = arrayStrings[2].slice(0, 10);
			//console.log(arrayStrings[2])
			//console.log(escState)
			//console.log(escArray[i])

        	if (escState !== 'in Betrieb') {
        		if (feature.properties.tags.highway === 'elevator'){
					return L.marker(lalo, {icon: aufzugAnimationOOR})
				}else{
					return {className: 'conveyingForwardOOR'}
				}
        	} 
        	if (escState === 'in Betrieb') {
        		if (feature.properties.tags.highway === 'elevator'){
					return L.marker(lalo, {icon: aufzugAnimation})
				}else if (feature.properties.tags.conveying === 'reversible'){
					return {className: 'conveyingRev'}
				}else if (feature.properties.tags.conveying === 'yes'){
					return {className: 'conveyingRev'}
				}else {
					return {className: 'conveyingForward'}
				}
        	}
        	//if (typeof escState === 'undefined') {
            //	return {className: 'conveyingForward'};
        	//}
		}
	} if (escArray[i] != true){
		if (feature.properties.tags.highway === 'elevator'){
			return L.marker(lalo, {icon: aufzugAnimation})
		}else if (feature.properties.tags.highway === 'steps'){
			return {className: 'steps'}
		}
	};
}; //Ende getEscalatorStatus

function escPopup(feature, layer){
	// Schleife um Rolltreppen-Array zu durchlaufen
	for (var i = 0; i < escArray.length; i++) {
		//IF-Abfrage um Array-Value zu bestimmen
		if (escArray[i].indexOf(feature.properties.tags.description)> -1) {
			...
			splitArrayString(escArray[i], ': ');
			escState = arrayStrings[2].slice(0, 10);
			//escStateStyle(escState);
			//console.log(escState)

			//popupContentEscalator = '<caption><h6><b>' + escArray[i] + '</b></h6></caption>';
			// Slice: Schneide hinten was ab.
			popupContentEscalator = '<caption><h6><b>' + arrayStrings[0].slice(0, -5) + '</b></h6></caption>';
			popupContentEscalator += '<tr><td>' + arrayStrings[1].slice(0, -7) + '</tr></td>';
			popupContentEscalator += '<tr><td><i>Status: &nbsp;</i>' + arrayStrings[2].slice(0, -6) + '</tr></td>';
			popupContentEscalator += '<tr><td><i>Stand: &nbsp;</i>' + arrayStrings[3] + '</tr></td>';
			
			layer.bindPopup('<table>' + popupContentEscalator + '</table>');
			//console.log("popupContentEscalator: " + popupContentEscalator)
		};
	//console.log("a: " + escArray[i].indexOf(description)>-1);
	//console.log("escArray: " + escArray[i])
	};
};
...
/*var aufzugAnimation = L.icon({
	className: 'aufzugsPfeile',
    iconUrl: 'assets/src/Aufzug.svg',
    iconSize: [30, 50]
});*/

var aufzugAnimation = L.divIcon({
	className: 'aufzugsPfeile',
    iconSize: null,
    iconAnchor: [12,12],
    html: '<div class="svg-container">\
  <svg class="aufzug" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="61.9" height="70" viewBox="0 0 61.9 70" overflow="hidden" enable-background="new 0 0 61.867 70" xml:space="preserve">\
	  <g class="pfeilUnten">\
		  <polygon stroke="" stroke-linejoin="round" points="47.2,23.3 54.3,11.9 61.4,0.5 47.2,0.5 33,0.5 40.1,11.9"/>\
	  </g>\
    <g class="pfeilOben">\
		  <polygon stroke="" stroke-linejoin="round" points="14.7,56.1 7.6,67.5 0.5,78.9 14.7,78.9 28.9,78.9 21.8,67.5"/>\
    </g>\
  </svg>\
</div>'
});

var aufzugAnimationOOR = L.divIcon({
	className: 'aufzugsPfeileOOR',
    iconSize: null,
    iconAnchor: [12,12],
    html: '<div class="svg-container">\
  <svg class="aufzugOOR" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="61.9" height="70" viewBox="0 0 61.9 70" overflow="hidden" enable-background="new 0 0 61.867 70" xml:space="preserve">\
	  <g class="pfeilUnten">\
		  <polygon stroke="" stroke-linejoin="round" points="47.2,33.3 54.3,21.9 61.4,10.5 47.2,10.5 33,10.5 40.1,21.9"/>\
	  </g>\
    <g class="pfeilOben">\
		  <polygon stroke="" stroke-linejoin="round" points="14.7,36.1 7.6,47.5 0.5,58.9 14.7,58.9 28.9,58.9 21.8,47.5"/>\
    </g>\
  </svg>\
</div>'
});
...
function escalatorData(escName){
...
// Test 
/*console.log(escName)
$.get("http://localhost/mvg", function(data){
		console.log(data);
});*/

// Test alle Bhfs Abfragen
/*
$.ajax({
  url: "http://localhost/mvg",
  type: "GET",
  crossDomain: true,
  dataType: "xml",
  //data: data,
  	success: function(data){
			var tagName = data;
			var nameArray = new Array(tagName.getElementsByTagName("area"))
			console.log($(tagName['area']).text())
			if(tagName.getElementsByTagName("area").toString().contains(escName)){
				console.log(escName)
			}

			for (var i = 0; i < tagName.length; i++) {
				//IF-Abfrage um Array-Value zu bestimmen
				if (escArray[i].contains(escName)) {
					console.log(true)
				}}
			escArray = new Array()
			for (var i = 0; i < tagData.getElementsByTagName("area").length; i++) {
...
// Spezifischen Bhf abfragen
$.ajax({
  //crossOrigin: true,
  //url: "http://www.mvg-zoom.de/fahrgastinfo/do?_flowId=showStation&id=SB",
  url: "/mvg/fahrgastinfo/do?_flowId=showStation&id=SB",
  type: "GET",
  crossDomain: true,
  dataType: "xml",
  //data: data,
...
//String Wert des Array wird noch mal in ein Array mit Strings geteilt:
function splitArrayString(escArrayString, separator){
	arrayStrings = escArrayString.split(separator);
}